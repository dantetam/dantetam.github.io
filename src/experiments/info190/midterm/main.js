...
d3.csv(dataPath, renderCallback);  
  
function loadData(data, varNameX, varNameY, varNameColor) {
  ...
  //Remove data not being shown, in the exit()
  circles.exit()
    .transition(t)
    .attr('r', 0)
...
//This architecture is to only load all of the data once, and then access it accordingly
function renderCallback(err, data) {
  ...
  //Cycle through all possible combinations of x and y variables
  ...
  //This callback has access to all the data, within scope
  allVarCombinations.forEach(function(item, i) {
    d3.timeout(function() {
      loadData(data, item[0], item[1], "year")
    }, 3000 * i);
  });
  
  //loadData(data, "acceleration", "cylinders", "year")
