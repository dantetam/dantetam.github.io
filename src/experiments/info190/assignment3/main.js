...
// Much like the "stroke()" function in processing/processing.js, 
// this sets the outline color,
// and then creates a rectangle. Note the coords using top-left as the origin
...
// This is the "true" width of the graphic, displayed in a larger screen
width = width - margin.left - margin.right;
height = height - margin.top - margin.bottom;

// translate the coordinate system origin by x & y pixels
// All future computations are shifted by this offset
context.translate(margin.left, margin.top);

// Much like "fill()" in Processing
...
var data = [];
var normalGenerator = d3.randomNormal(100, 50);
for (var i = 0; i < 50; i++) {
  data.push(normalGenerator());
}

// create a new y scale function and set its domain and range
// we're using scaleLinear because our values are continuous

//These functions serve to map our input data to the screen coordinates,
//which will be used in the generation of the bar plot graphics.
...
// This is much like the "Turtle" pen in Python, 
// which can move its pen up and down,
// meaning that all draws have a start and stop point
...
// Note that d3.js/JS has quite a few convenience functions,
...
// Render these bar labels at an angle
...
xScale.domain().forEach(function(d, i) {
  //Format the bar labels such that they truncate up to two decimal places
  var formatter = d3.format(".2f");
  var formatted = formatter(d);  
  
  context.translate(xScale(d) + xScale.bandwidth() / 2, yScale(d) - 15);
  context.rotate(- Math.PI * 0.375);
  
  context.fillText(formatted, 0, 0);
...
context.textAlign = 'right';
context.textBaseline = 'top';
context.fillText('Y ~ Normal(mean=200, standard-deviation=50)', -5, 5);
